...
<div class="pm-header">
    <p class="pm-overline">Competition payment</p>
    <p style="margin: 0;font-size: 1.3rem;">
        <?= out($comp->name) ?>
    </p>
    <p class="pm-subtitle">
        <?= out($comp->location) ?> • <?= out($comp->year) ?>
    </p>
</div>
...
<section class="pm-body">
    <div class="pm-summary">
        <div class="pm-row">
            <span>Participants (locked):</span>
            <strong>
                <?= (int) ($comp->billing_participants_locked ?? 0) ?>
            </strong>
        </div>
        <div class="pm-row">
            <span>Pricing tier:</span>
            <strong>
                <?= out($pricing['label'] ?? $pricing['tier'] ?? '') ?>
            </strong>
        </div>
        <div class="pm-row">
            <span>Amount:</span>
            <strong>
                <?= number_format($pricing['amount'] ?? 0, 2) ?> <i class="fa fa-euro" style="font-weight: 900;" aria-hidden="true"></i>
            </strong>
        </div>
        <div class="pm-row">
            <span>Status:</span>
            <?php
                $status = strtolower($comp->billing_status ?? 'not_required');
    ...
    <p class="pm-note">
        After payment your participant count and pricing tier will be locked
        for this competition. You’ll unlock heat generation and live judging.
    </p>
</section>
...
<div class="pm-footer">
    ...
    <?php else: ?>
        <a href="<?= out($payment_link) ?>" class="btn secondary">
             Pay <?= number_format($pricing['amount'] ?? 0) ?> <i class="fa fa-euro" style="font-weight: 900;" aria-hidden="true"></i>
        </a>
    <?php endif; ?>
</div>
